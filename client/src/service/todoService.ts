...
export const callGetTodoById = async (id: string | undefined) => {
  try {
    const res = await axios.get(`${BASE_URL}/${id}`, header);
    if (res.status === 200) {
      return res;
    }
  } catch (error: any) {
...
export const callCreateTodo = async (params: TodoInput) => {
  try {
    const res = await axios.post(`${BASE_URL}`, params, header);
    if (res.status === 200) {
      return res;
    }
  } catch (error: any) {
...
export const callUpdateTodo = async (id: string | undefined, params: Todo) => {
  try {
    const res = await axios.put(`${BASE_URL}/${id}`, params, header);
    if (res.status === 200) {
      return res;
    }
  } catch (error: any) {
...
export const callDeleteTodo = async (id: string) => {
  try {
    const res = await axios.delete(`${BASE_URL}/${id}`, header);
    if (res.status === 200) {
